...
class TestExperiment(unittest.TestCase):
    ...
    def test_with_ratio_split(self):
        Experiment(
            ...
            verbose=True,
            rerank=0,
            # diversified_metrics=[ILD(item_feature=Item_feature)],
            # reranking_metrics=[ILD(item_feature=Item_feature)]
        ).run()
    ...
    def test_with_diversity(self):
        ...
        #    metrics=[bino, cal, ndcg, gini, eild,
        #             ild, act, repre, alt, frag],
        Experiment(eval_method=ratio_split, models=[UserKNN(k=3, similarity="pearson", name="UserKNN"), PMF(1, 0)],
                   # the number of candidate setrerank=0,  # the number of re-ranking items
                   metrics=[bino, ndcg], k=2,
                   lambda_constant=0,
                   rerank=0,
                   diversified_metrics=[ILD(item_feature=Item_feature)],
                   reranking_metrics=[ILD(item_feature=Item_feature)]
